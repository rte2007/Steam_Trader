# -*- coding: utf-8 -*-
"""market.csgo.com API v2 helpers for the CS2 P2P sale route.
...
MARKET_CSGO_BASE = "https://market.csgo.com/api/v2"
...
def _req(method: str, path: str, params: dict | None = None, json_body=None, retries: int = 0, timeout: int = 20):
    ...
    params["key"] = MARKET_CSGO_API_KEY
    attempt = 0
    while True:
        try:
            r = requests.request(method, f"{MARKET_CSGO_BASE}/{path}", params=params, json=json_body, timeout=timeout)
            break
...
def get_prices(currency: str = "USD") -> dict[str, dict]:
    ...
    """
    r = requests.get(f"{MARKET_CSGO_BASE}/prices/class_instance/{currency}.json", timeout=20)
    ...
    for name, row in result.items():
        row["buy_order"] = float((orders.get(name) or {}).get("price") or 0)
        row["buy_order_volume"] = int((orders.get(name) or {}).get("volume") or 0)
...
def get_buy_orders(currency: str = "USD") -> dict[str, dict]:
    ...
    try:
        r = requests.get(f"{MARKET_CSGO_BASE}/prices/orders/{currency}.json", timeout=20)
    except requests.exceptions.RequestException:
        return {}
    if r.status_code != 200:
        return {}
